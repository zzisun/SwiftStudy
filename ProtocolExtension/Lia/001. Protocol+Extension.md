...
3. Static Dispatch  vs   Dynamic Dispatch
   ...
   - Dynamic Dispatch: 프로토콜에 메소드나 프로퍼티를 정의하지 않고, extension에서만 정의한 경우
     컴파일  타임에 어떤 메소드가 호출될 지 모르기 때문에, 런타임에 프로토콜의 extension에 구현된 메소드를 호출한다
...
출처 : 스위프트 프로그래밍 (야곰 저), [동적 vs 정적 디스패치](http://minsone.github.io/programming/swift3-if-protocol-extension-and-concreated-impletement-have-same-method), Swift Language Guide
